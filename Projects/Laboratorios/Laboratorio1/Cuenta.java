...
/**
 * Implements a class storing bank account information.
 * A bank account (cuenta) consists of 
 * a <em>unique</em> identifier, a DNI identifying the customer,
 * and a bank balance (saldo).
 */
public class Cuenta {
  ...
  /**
   * Creates a new bank account, with a <em>unique</em>
  ...
  /**
   * Increases the balance (saldo) of the bank account with cantidad,
  ...
  /**
   * Decreases the balance (saldo) of the bank account with cantidad,
   * and returns the new balance.
   * @return the new balance.
  ...
  public int retirar(int cantidad) throws InsuficienteSaldoExc {
    if (cantidad < 0) throw new IllegalArgumentException();
    if (saldo < cantidad) throw new InsuficienteSaldoExc();
    saldo -= cantidad;
    return saldo;
  }
